...
E64::timer_ic::timer_ic(TTL74LS148_ic *unit)
{
	TTL74LS148 = unit;
	/*
	 * TODO
	 */
	interrupt_device_no = TTL74LS148->connect_device(2);
}
...
void E64::timer_ic::run(uint32_t number_of_cycles)
{
	for (int i=0; i<8; i++) {
		...
		if ((timers[i].counter >= timers[i].clock_interval) && (control_register & (0b1 << i))) {
			...
			TTL74LS148->pull_line(interrupt_device_no);
...
uint8_t E64::timer_ic::io_read_8(uint8_t address)
{
	switch (address & 0x1f) {
		case 0x00:
...
void E64::timer_ic::io_write_8(uint8_t address, uint8_t byte)
{
	switch (address & 0x1f) {
		case 0x00:
			/*
...
void E64::timer_ic::set(uint8_t timer_no, uint16_t bpm)
{
	...
	uint8_t byte = io_read_8(0x01);
	io_write_8(0x01, (0b1 << timer_no) | byte);
}

void E64::timer_ic::status(char *buffer, int n)
{
	for (int timer_no=0; timer_no<8; timer_no++) {
